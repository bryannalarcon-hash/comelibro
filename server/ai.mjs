...
const promptVersions = { help: 'comelibro-help-v1', curriculum: 'comelibro-curriculum-v5', review: 'comelibro-review-v4' };
...
export function applyReview(proposal, review) {
  ...
  for (const q of questions) {
    ...
    q.review = { status: receipt.status === 'approved' && ['sourceGrounding', 'spanishAccuracy', 'answerKey', 'objectiveAlignment'].every(k => receipt[k] === true) ? 'approved' : 'uncertain', reviewer: `${AI_MODEL}:independent-semantic-v4`, reason: receipt.reason, version: q.version };
  }
  return proposal;
}
...
export async function runTask({ kind, input, signal, onProgress = () => {}, onEvent = () => {} }) {
  ...
  try {
    ...
    for (const lesson of result.lessons) for (const q of lesson.questions) q.review = { status: 'uncertain', reviewer: 'independent-semantic-v4:incomplete', reason: 'Independent review did not finish. An administrator must review this exact item before it can score mastery.', version: q.version };
